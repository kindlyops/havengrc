...
// override the temp directory so the compiler doesn't litter our working dir with temp
// compiled files
...
const highlightElmCompilerErrors = require('./utils/formatElmCompilerErrors');
...
function printInstructions(appName, urls) {
  ...
  console.log(
    `To create a production build, contact ` + `${chalk.cyan('support@kindlyops.com')}.`
  );
  console.log();
}
...
function createCompiler(webpack, config, appName, urls) {
  ...
  compiler.hooks.done.tap('done', stats => {
    ...
    if (isSuccessful) {
      console.log(chalk.green('Compiled SO VERY WOW successfully!'));
    }
    ...
    if (messages.errors.length) {
      console.log(chalk.red('SO SAD SO SAD Failed to compile.\n'));
...
const DEFAULT_PORT = parseInt(process.env.PORT, 10) || 2015;
